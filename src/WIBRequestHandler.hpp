...
protected:
  ...
  RequestResult tpc_data_request(dfmessages::DataRequest dr, unsigned delay_us)
  {
    ...
    dataformats::WIBHeader front_wh = *(reinterpret_cast<const dataformats::WIBHeader*>( m_latency_buffer->get_ptr(0) )); // NOLINT
      ...
      for (uint32_t idxoffset = 0; idxoffset < num_elements_in_window; ++idxoffset) { // NOLINT(build/unsigned)
        ...
        if (element != nullptr) {
          frag_pieces.emplace_back(
            std::make_pair<void*, size_t>(static_cast<void*>(m_latency_buffer->get_ptr(num_element_offset + idxoffset)),
