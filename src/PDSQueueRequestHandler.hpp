...
protected:
  ...
  RequestResult pds_data_request(dfmessages::DataRequest dr, unsigned delay_us)
  {
    if (delay_us > 0) {
    ...
    if (rres.result_code != ResultCode::kFound) {
      ...
      for (int index = start_idx; index != m_latency_buffer->next_index(last_chunk);
        ...
        if (element != nullptr) {
          frag_pieces.emplace_back(std::make_pair<void*, size_t>(static_cast<void*>(m_latency_buffer->at(index)),
